...
.al-root{
  --paper:#0F1A2E; --surface:#18243C; --surface2:#20304F; --ink:#EEF3FA; --muted:#93A2BD;
  --line:#2B3A57; --coral:#FF5C61; --coral-d:#FF8A7E; --teal:#1DD3B0;
  --amber:#FBB024; --violet:#8B7BF7;
  --serif:'Fraunces',Georgia,serif; --sans:'Inter',system-ui,sans-serif;
  --mono:'Space Mono','SF Mono',ui-monospace,monospace;
  font-family:var(--sans); color:var(--ink);
  background:
    radial-gradient(1100px 520px at 12% -8%, rgba(251,176,36,.16) 0%, transparent 55%),
    radial-gradient(900px 520px at 100% 0%, rgba(29,211,176,.13) 0%, transparent 52%),
    radial-gradient(760px 520px at 50% 118%, rgba(139,123,247,.12) 0%, transparent 60%),
    var(--paper);
...
.al-ipa-strip{font-family:var(--mono);color:#46577C;font-size:15px;letter-spacing:.28em;
...
.al-card{background:var(--surface);border:1px solid var(--line);border-radius:18px;
  box-shadow:0 1px 0 rgba(255,255,255,.03),0 20px 44px -26px rgba(0,0,0,.75);}
.al-pad{padding:22px;}
...
.al-btn:focus-visible{outline:3px solid rgba(251,176,36,.6);outline-offset:2px;}
.al-primary{background:linear-gradient(135deg,var(--coral),#FF7E3F);color:#fff;box-shadow:0 10px 26px -8px rgba(255,92,97,.6);}
.al-primary:hover{filter:brightness(1.08);}
.al-ghost{background:rgba(255,255,255,.07);color:var(--ink);}
.al-ghost:hover{background:rgba(255,255,255,.13);}
.al-teal{background:var(--teal);color:#05261F;}
.al-teal:hover{filter:brightness(1.08);}
.al-mini{padding:9px 13px;font-size:13.5px;border-radius:10px;}
.al-btn[disabled]{opacity:.5;cursor:not-allowed;}

.al-tabs{display:flex;flex-wrap:wrap;justify-content:center;gap:7px;background:rgba(255,255,255,.05);border:1px solid var(--line);
...
.al-tab.on{background:var(--surface2);color:var(--ink);box-shadow:0 0 0 1px rgba(251,176,36,.4),0 8px 16px -8px rgba(0,0,0,.6);}

.al-chip{display:inline-flex;align-items:center;gap:7px;background:rgba(29,211,176,.14);color:var(--teal);
  border:1px solid rgba(29,211,176,.4);font-weight:700;font-size:13px;padding:6px 12px;border-radius:999px;}
...
.al-vs{font-family:var(--mono);color:#5B6E92;font-size:13px;font-weight:700;}
.al-slot{background:var(--surface2);border:1px solid var(--line);border-radius:14px;padding:14px;text-align:center;}
...
.al-opt{display:block;width:100%;text-align:left;background:var(--surface2);border:1.5px solid var(--line);
...
.al-opt:hover{border-color:#3D5480;background:#26375A;}
.al-opt.sel{border-color:var(--coral);background:rgba(255,92,97,.16);box-shadow:0 0 0 3px rgba(255,92,97,.3);}

.al-progress{height:7px;background:rgba(255,255,255,.09);border-radius:99px;overflow:hidden;margin-bottom:22px;}
.al-progress i{display:block;height:100%;background:linear-gradient(90deg,var(--coral),var(--amber));
  border-radius:99px;transition:width .3s ease;}

.al-tip{background:rgba(251,176,36,.12);border:1px solid rgba(251,176,36,.32);border-radius:12px;padding:12px 14px;
  font-size:13.5px;color:#F3D89B;line-height:1.5;}
.al-tip b{color:var(--amber);}
...
.al-spin{width:18px;height:18px;border:2.5px solid rgba(255,255,255,.15);border-top-color:var(--coral);
...
.al-sent .un{color:#6E80A3;font-weight:400;font-size:19px;}
...
.al-bub.tut{align-self:flex-start;background:var(--surface2);border:1px solid var(--line);border-bottom-left-radius:5px;}
.al-bub.me{align-self:flex-end;background:linear-gradient(135deg,var(--coral),#FF7E3F);color:#fff;border-bottom-right-radius:5px;}
.al-fix{font-family:var(--sans);font-size:13px;color:var(--teal);margin-top:6px;display:block;}
.al-input{flex:1;background:var(--surface2);color:var(--ink);border:1.5px solid var(--line);border-radius:12px;padding:12px 14px;font-size:15px;
  font-family:var(--sans);outline:none;}
.al-input::placeholder{color:var(--muted);}
.al-input:focus{border-color:var(--coral);box-shadow:0 0 0 3px rgba(255,92,97,.25);}

/* stats */
.al-stat{background:var(--surface2);border:1px solid var(--line);border-radius:14px;padding:16px;text-align:center;}
...
.al-bar{height:12px;background:rgba(255,255,255,.09);border-radius:99px;overflow:hidden;margin:6px 0 12px;}
        ...
        return (
          <div key={i} className="al-card al-pad" style={{ marginBottom: 12 }}>
            <div className="al-row" style={{ justifyContent: "space-between", alignItems: "flex-start" }}>
              ...
              <span className="al-chip" style={{ background: "rgba(139,123,247,.16)", color: "var(--violet)", borderColor: "rgba(139,123,247,.42)" }}>{it.register}</span>
            </div>
  ...
  return (
    <div className="al-fade">
      ...
      <div className="al-card al-pad">
        ...
        <div className="al-q" style={{ marginTop: 12 }}>
          {item.q.split("___").map((part, i) => (
            <React.Fragment key={i}>
              {part}
              {i === 0 && <span style={{ color: answered ? (correct ? "var(--teal)" : "var(--coral)") : "var(--muted)", fontWeight: 700 }}>
        ...
        <div className="al-row">
          {item.opts.map((o, i) => {
            let bg = "var(--surface2)", bd = "var(--line)", col = "var(--ink)";
            if (answered) {
              if (i === item.a) { bg = "rgba(29,211,176,.16)"; bd = "var(--teal)"; col = "var(--teal)"; }
              else if (i === chosen) { bg = "rgba(255,92,97,.16)"; bd = "var(--coral)"; col = "var(--coral-d)"; }
            }
            return (
            ...
            <div className="al-brand" style={{ justifyContent: "space-between", width: "100%" }}>
              ...
              <span className="al-chip">
                ...
                <select value={progress.level} onChange={(e) => setLevel(e.target.value)}
                  style={{ border: "none", background: "transparent", color: "var(--teal)", fontWeight: 700, fontFamily: "var(--sans)", cursor: "pointer" }}>
